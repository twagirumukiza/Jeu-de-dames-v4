...
function listenRoom() {
  ...
  unsub = onValue(ref(db, `damesRooms/${room}`), async snap => {
    ...
    // Dès que la partie commence, les contrôles de connexion se replient
    // pour laisser toute la place au damier. Ils réapparaissent au lobby/à la fin.
    $('#onlinePanel').classList.toggle('gameRunning', onlineStarted);
...
function leaveOnlineState() {
  ...
  $('#onlinePanel').classList.remove('gameRunning');
