...
async function createRoom() {
  try {
    ...
    room = code();
    B = freshBoard(); turn = 1; chain = false; sel = null; onlineStarted = false; roomHasTwo = false; lastWinnerShown = 0; lastMoveId = null;
    const data = {
      hostId: myUid,
...
async function startOnlineGame() {
  ...
  try {
    ...
    onlineMsg('Impossible de lancer : ' + friendlyFirebase(e));
  }
}
...
function listenRoom() {
  ...
  unsub = onValue(ref(db, `damesRooms/${room}`), async snap => {
    ...
    const g = d.game || {};
    const wasPlaying = onlineStarted;
    onlineStarted = g.status === 'playing';
    if (onlineStarted && !wasPlaying) {
      // La partie vient de démarrer (ou redémarrer) : on déverrouille tout
      locked = false;
      lastMoveId = null;
      lastWinnerShown = 0;
      applyingRemote = false;
